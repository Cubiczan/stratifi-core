...
from cme.research.data.filing_sweep import FilingSweepReport, run_filing_sweep
...
def _cmd_filing_sweep(args: argparse.Namespace) -> int:
    """Run a comprehensive 8-type SEC filing sweep."""
    edgar = EdgarClient()
    report = run_filing_sweep(
        edgar,
        args.ticker,
        s1_limit=args.s1_limit,
        form_13f_limit=args.form13f_limit,
        form_d_limit=args.form_d_limit,
        pre14a_limit=args.pre14a_limit,
        standard_limit=args.standard_limit,
        cik_override=args.cik_override,
    )
    print(report.render())
    return 0
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    fs = sub.add_parser("filing-sweep", help="Comprehensive 8-type SEC filing sweep via EDGAR")
    fs.add_argument("--ticker", required=True, help="Stock ticker or 13F manager org (use with --cik-override)")
    fs.add_argument("--cik-override", type=int, default=None, help="Explicit CIK for 13F manager lookups (e.g. 1067983 for Berkshire)")
    fs.add_argument("--s1-limit", type=int, default=3, help="Max S-1 filings to fetch")
    fs.add_argument("--form13f-limit", type=int, default=2, help="Max 13F filings to fetch")
    fs.add_argument("--form-d-limit", type=int, default=3, help="Max Form D filings to fetch")
    fs.add_argument("--pre14a-limit", type=int, default=3, help="Max PRE 14A filings to fetch")
    fs.add_argument("--standard-limit", type=int, default=5, help="Max 10-K/10-Q/DEF 14A filings to fetch")
    fs.set_defaults(func=_cmd_filing_sweep)
